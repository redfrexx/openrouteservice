 ...
 */
public class NewGreenIndexGraphStorage implements GraphExtension {
    /* pointer for no entry */
    protected final int NO_ENTRY = -1;
    private final int EF_greenIndex;

    private DataAccess orsEdges; // Data store for green index value of edges
    private int edgeEntryBytes;
    private int edgesCount; // number of edges with custom values

    private byte[] byteValues;

    public NewGreenIndexGraphStorage() {
        EF_greenIndex = 0;
    ...
    public void setEdgeValue(int edgeId, byte greenIndexValue) {
        edgesCount++;
        ensureEdgesIndex(edgeId);

        // add entry
        long edgePointer = (long) edgeId * edgeEntryBytes;
        byteValues[0] = greenIndexValue;
    ...
    public GraphExtension copyTo(GraphExtension clonedStorage) {
        if (!(clonedStorage instanceof NewGreenIndexGraphStorage)) {
            throw new IllegalStateException("the extended storage to clone must be the same");
        }

        NewGreenIndexGraphStorage clonedTC = (NewGreenIndexGraphStorage) clonedStorage;
